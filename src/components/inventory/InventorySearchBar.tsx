...
const InventorySearchBar = ({ q }: { q?: string }) => {
        ...
        <form action="/dashboard/inventory" method="GET">
          <div className="flex rounded-md border-2 border-blue-500 overflow-hidden ">
            <input
              ...
              className="w-full outline-none bg-white text-gray-600 text-sm px-4 py-5"
              autoFocus
            />
            {search && (
              <button
                ...
                className=" w-8 h-8 rounded-full bg-red-100 flex items-center justify-center  absolute top-4 right-20 "
              >
                ❌
              </button>
            )}
